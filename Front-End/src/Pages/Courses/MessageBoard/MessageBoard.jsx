...
import { Comment, Divider, Button, Card, message, Tooltip, Input, Modal, notification, Tag, Spin, Pagination } from 'antd'
import {LikeOutlined ,DeleteOutlined,UpCircleOutlined,DownCircleOutlined} from'@ant-design/icons'
...
class MessageBoard extends Component {
    ...
    getMessages = async (page = 1, pageSize = 10) => {
        ...
        let res = data;
        if(this.state.datakind === 1){
            res = data1
        }
        else if(this.state.datakind === 2){
            res = data2
        }

        /** await json.get('/message/list', {
            current: page - 1,
            pageSize: pageSize
        })*/
        if (res.status !== 0) {
    ...
    sendMessage = async () => {
        ...
        const res = data1
            /**await json.post('/message/create', {
            content: htmlContent
        })*/
        if (res.status === 0) {
    ...
    confirmReply = async (item) => {
        ...
        const res = data
        /**await json.post('/message/create', param)*/
        if (res.status === 0) {
    ...
    onDelete = async (item) => {
        Modal.confirm({
            ...
            onOk: async () => {
                const res = data
                /**await json.post('/message/delete', {
                    id: item.id
                })*/
                if (res.status === 0) {
                    ...
                    <div>
                        {
                            isShowEditor ? (
                                ...
                                <p>&nbsp;</p>
                                <Button onClick={() => this.setState({ isShowEditor: true ,datakind:1})}>我要发帖</Button>
                            </div>
                        }
                    </div>
                    <Divider />
                                    ...
                                    <div className='toggle-reply-box' style={{ display: item.children.length > 1 ? 'block' : 'none' }}>
                                        {
                                            expandIds.includes(item.id) ? (
                                                <span onClick={() => this.foldReply(item)}>收起全部{item.children.length}条回复 <UpCircleOutlined /></span>
                                            ) : (
                                                <span onClick={() => this.expandReply(item)}>展开全部{item.children.length}条回复 <DownCircleOutlined /></span>
                                            )
                                        }
                                    </div>
                                    {replyPid === item.id && (
