...
class Course(models.Model):
    ...
    course_info = models.CharField(max_length=512, verbose_name=u'课程简介', default=' ')
    create_time = models.DateTimeField(auto_now_add=True, verbose_name=u'创建时间')  # 第一次保存时记录时间
    modify_time = models.DateTimeField(auto_now=True, verbose_name=u'修改时间')
...
class Section2Teacher(models.Model):
    section_user_id = models.AutoField(primary_key=True, verbose_name=u'课程教师关系编号')
    section_id = models.ForeignKey(Section, on_delete=models.DO_NOTHING, verbose_name=u'教学班编号')
    user_id = models.ForeignKey(User, on_delete=models.DO_NOTHING, verbose_name=u'用户学工号')

    class Meta:
        # managed = False     # false 为False的时候，不会对数据库表进行创建、删除等操作 可以用于现有表、数据库视图等
        db_table = 'section2teacher'
        verbose_name_plural = '教学班教师表'


class Section2Student(models.Model):
    section_user_id = models.AutoField(primary_key=True, verbose_name=u'课程学生关系编号')
    section_id = models.ForeignKey(Section, on_delete=models.DO_NOTHING, verbose_name=u'教学班编号')
    user_id = models.ForeignKey(User, on_delete=models.DO_NOTHING, verbose_name=u'用户学工号')

    class Meta:
        # managed = False     # false 为False的时候，不会对数据库表进行创建、删除等操作 可以用于现有表、数据库视图等
        db_table = 'section2student'
